...
if(this.options.images.length>0){var c=null,e,d;
...
this._loadImages();
this._loadSlider();
if(this.options.categories.enabled){this._loadCategoryTitles()
...
},destroy:function(){this._$images.each(function(d,c){b(c).cover("destroy")
});
...
}},_playNext:function(){if(this.options.categories.enabled){if(this._playCountInCategory>this._imagesCount()||this._playCountInCategory>=this.options.autoplay.playsPerCategory-1){this._nextCategory();
...
e<this._categories.length;
e++){if(this._categories[e]==g){this._currentCategoryIndex=e;
break
...
}},_clickCover:function(d,c){this._gotoCover(c.image.data("coverflow").index)
